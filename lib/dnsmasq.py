...
class IPAMasq:
  def __init__(self,dbfile="ipamasq.db"):
    pass

class DnsMasqBase:
  def __init__(self,config,pidfile,logfile,savedconfig=""):
    self.SWITCHES = []
    self.CONF = {}
    ...
    if os.path.exists(savedconfig):
      self.ReadConfig(savedconfig)
  ...
  def ReadConfig(self,conf):
    ...
    for this_line in config_lines:
      if re.match(r'#',this_line):
        continue
      elif re.match(r'=',this_line):
        opt = this_line.split('=')
        self.Set(opt[0],opt[1])
      else:
        self.Set(this_line)
...
class DnsMasqDHCP(DnsMasqBase):
  def __init__(self,interface="eth0"):
    super().__init__(config="dnsmasq_dhcp.conf",pidfile="dnsmasq_dhcp.pid",logfile="dnsmasq_dhcp.log",savedconfig=savedconfig)

    self.Set("dhcp-leasefile","dnsmasq.leases")
...
class DnsMasqTFTP(DnsMasqBase):
  def __init__(self,tftppath="/var/ftpd",secure=True,interface="eth0",savedconfig=""):
    super().__init__(interface=interface,config="dnsmasq_tftp.conf",logfile="dnsmasq_tftp.log",pidfile="dnsmasq_tftp.pid",savedconfig=savedconfig)
...
class DnsMasqPXE(DnsMasqDHCP):
  def __init__(self,tftppath="/var/ftpd",secure=True,interface="eth0",savedconfig=""):
    super().__init__(interface=interface,config="dnsmasq_pxe.conf",pidfile="dnsmasq_pxe.pid",logfile="dnsmasq_pxe.log",savedconfig=savedconfig)
...
class DnsMasqDNS(DnsMasqBase):
  def __init__(self,interface="eth0",port=53,savedconfig=""):
    super().__init__(config="dnsmasq_dns.conf",pidfile="dnsmasq_dns.pid",logfile="dnsmasq_dns.log",savedconfig=savedconfig)
  ...
  def AddLocalDomain(self,d):
    if not d in self.dns_conf['local_domains']:
      self.Set("local",d)
  
  def AddServer(self,svr,suffix=""):
